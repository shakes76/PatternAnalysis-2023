...
# Project Overview
The project aims to classify brain scans in the Alzheimer’s Disease Neuroimaging Initiative (ADNI) brain dataset as being either Cognitive Normal (NC) or suffering from Alzheimer's disease (AD). 
It achieves this through representation learning using a **Siamese network** with a twin **embedding network** structure.

A Siamese network is a neural network architecture that consists of two or more identical subnetworks that share the same model weights. 
In the present project's case, the Siamese architecture allows the **embedding network** to learn to generate similar feature vectors for images of the same class and dissimilar feature vectors for images of different classes. Once the **embedding network** is trained, a separate **classifier** is trained to classify the feature vectors generated by the **embedding network**.
...
The feature vector is then passed through the trained **classifier** to generate a prediction. The project is implemented in Python 3.11.4 using Pytorch (see below for other dependencies). It is capable of achieving a final classification accuracy of approximately 78% on the test set.
...
>
> Visualisation of the ADNI dataset (sample inputs) are provided in the section ***The Dataset and Preprocessing*** in the Appendix. The training, validation and testing split is also addressed in the same section.
>
...
## Defined Terms
The following terms are given specific meanings: 
* torch: Pytorch
...
The accuracy of the classifier for both datasets appears to remain stable throughout all epochs. Comparatively, the testing set has a greater range of variation. 
**Ultimately, the best accuracy achieved by the classifier on the test set is 78.46%.**
...
A potential explanation for this behaviour is the fact that the training and validation sets in this project are split deterministicly. 
This means that, as a matter of chance, the validation dataset might have contained slightly different patterns compared to the training dataset. 
Thus, the project can be further improved by the implementation of N-fold cross-validation, which will allow more insight into the model's fit to the training data to be shown.

A second interesting observation is that there appears to be no correlation between the number of epochs of classifier training and the accuracy of the classifier. This absence of correlation appears consistently across several training runs, regardless of the number of epochs of training that the Siamese backbone received. 
...
[3] Anurag Singh Choudhary, "[Introduction and Implementation of Siamese Networks](https://www.analyticsvidhya.com/blog/2023/08/introduction-and-implementation-of-siamese-networks/)", Web Page (accessed 20/10/2023).
