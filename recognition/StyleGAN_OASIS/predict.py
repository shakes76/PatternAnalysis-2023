...
"""
def plot_synthetic(generator, n):
    generator.eval()
    z = torch.randn(n, 256)
    images = generator(z)
    plt.figure(figsize=(16,16))
    plt.axis("off")
    plt.imshow(np.transpose(vutils.make_grid(images[:n], nrow=int(np.sqrt(n)), padding=2, normalize=True),(1,2,0)))
    plt.title("Synthetically Generated Images")
    plt.show()
