...
def train(model: TripletNetwork, criterion: TripletLoss, optimiser: optim.Optimizer,
    ...
    Args:
        model (TripletNetwork): Siamese model
        criterion (TripletLoss): loss function
        optimiser (optim.Optimizer): optimiser
    ...
    Returns:
        dict: training and validation loss values
    """
    losses = {
        "train": [],
        "valid": []
...
def test(chkpt_path, model: TripletNetwork, criterion: TripletLoss,
    ...
    Args:
        chkpt_path (_type_): path to saved weights
        model (TripletNetwork): Siamese model
        criterion (TripletLoss): loss function
        device (str): cpu or cuda
    ...
    Returns:
        tuple: list of loss values, average loss and generated embeddings
    """
...
def train_classifier(classifier: BinaryClassifier, siamese: TripletNetwork, criterion, 
    ...
    Args:
        classifier (BinaryClassifier): classifier model
        siamese (TripletNetwork): siamese model
...
def test_classifier(classifier: BinaryClassifier, siamese: TripletNetwork, criterion, 
                    device, test_loader: DataLoader):
    """_summary_
    ...
    Returns:
        tuple: loss values, average loss
    """
    classifier.eval()
    siamese.eval()
    losses = []
    ...
    print(f"Total batches: {len(test_loader)}")
    try:
        for batch_no, (a, label) in enumerate(test_loader):
            # move the data to the GPU
            a, label = a.to(device), torch.unsqueeze(label.to(device), dim=1).float()
            # input image into siamese model to generate embedding
            a_embed = siamese.single_foward(a)
            # pass into classifier
            a_out = classifier(a_embed)
            # calculate the loss
            loss = criterion(a_out, label)
            # record the loss
            losses.append(loss.item())
            total_loss += loss.item()
...
def parse_user_args():
    ...
    parser.add_argument(
        "--test-c-s",
        type=str,
        help="Path to saved siamese to test classifier, REQUIRED TO USE --test-c",
        metavar="SIAM_FILE_PATH"
    )

    args = parser.parse_args()
    return args.train_s, args.train_c, args.test_s, args.test_c, args.test_c_s


def main():
    """Main function"""
    is_training_s, train_c_s_path, saved_s_path, saved_c_path, saved_c_s_path = parse_user_args()
    ...
    if saved_c_path is None:
        ...
        saved_c_path = save_path
    else:
        if saved_c_s_path is None:
            print("MUST PROVIDE SIAMESE SAVED MODEL PATH TO TEST CLASSIFIER")
            return
    
    siamese.load_state_dict(torch.load(saved_c_s_path))
    classifier.load_state_dict(torch.load(saved_c_path))
    # test the classifier
    test_set = TripletDataset(root="data/test", transform=transform, triplet=False)
    test_loader = DataLoader(test_set, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)

    test_losses, average_test_loss = test(classifier, siamese, criterion, device, test_loader)
    plot_test_loss(test_losses)
