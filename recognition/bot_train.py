...
def init():
    ...
    """
    validDataSet = dataset.ISICDataset(dataset_type='validation', transform=dataset.get_transform())
    validDataloader = DataLoader(validDataSet, batch_size=batchSize, shuffle=False)
    trainDataSet = dataset.ISICDataset(dataset_type='training', transform=dataset.get_transform())
