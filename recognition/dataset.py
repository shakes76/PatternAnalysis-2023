...
# Define a custom dataset class, inheriting from the torch.utils.data.Dataset class.
class ISICDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Retrieve an item (image and optionally a mask) by index.
...
def get_mask_transform():
    # Function to define and return the transformations to apply to the mask.
    # Mask usually don't require normalization, but they still need to be tensors.
    return transforms.Compose([
        transforms.ToTensor()  # Convert the mask to a PyTorch tensor.
    ])

# Define the transformation function for images.
def get_transform():
    # Define and return the transformations to apply to the images
    # Includes normalization, and the values are standard for pre-trained models.
    return transforms.Compose([
