...
class UNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_features=16):
    ...
    def forward(self, x):
        ...
        # Up 1
        if out.shape != concat_3.shape:
            out = TF.resize(out, size=concat_3.shape[2:], antialias=False)
        out = torch.cat([out, concat_3], dim=1)
        out = self.lm1(out)
        out = self.um1(out)

        # Up 2
        if out.shape != concat_2.shape:
            out = TF.resize(out, size=concat_2.shape[2:], antialias=False)
        ...
        # Up 3
        if out.shape != concat_1.shape:
            out = TF.resize(out, size=concat_1.shape[2:], antialias=False)
        ...
        # Up 4
        if out.shape != concat_0.shape:
            out = TF.resize(out, size=concat_0.shape[2:], antialias=False)
        out = torch.cat([out, concat_0], dim=1)
        out = self.final_cov(out)

        if seg_2.shape != seg_3.shape:
            seg_2 = TF.resize(seg_2, size=seg_3.shape[2:], antialias=False)

        seg_2_add_seg_3 = seg_2+seg_3
        seg_2_add_seg_3 = self.upsample(seg_2_add_seg_3)

        if out.shape != seg_2_add_seg_3.shape:
            seg_2_add_seg_3 = TF.resize(seg_2_add_seg_3, size=out.shape[2:])
