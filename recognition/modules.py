...
class ContextModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=(3,3,3), stride = 2, padding=1),
            nn.InstanceNorm3d(out_channels),
            nn.LeakyReLU(negative_slope=1e-2, inplace=True),
        )
        # Another 3x3x3 convolution layer followed by instance normalisation and leaky ReLU activation
        self.conv2 = nn.Sequential(
            nn.Conv3d(out_channels, out_channels, kernel_size=(3,3,3), stride = 2, padding=1),
            nn.InstanceNorm3d(out_channels),
            nn.LeakyReLU(negative_slope=1e-2, inplace=True),
        )
...
class LocalisationModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Conv3d(in_channels, out_channels, kernel_size=(3,3,3), stride=2, padding=1)
        # 1x1x1 convolution to reduce the number of feature maps
        self.conv2 = nn.Conv3d(out_channels, out_channels//2, kernel_size=(3,3,3), padding=1)
...
class UpsamplingModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=(3,3,3), stride=2, padding=1)
...
class UNet3D(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        # Encoder
        # Encoder Layer 1
        self.enc1 = nn.Conv3d(in_channels, 16, kernel_size=(3,3,3), stride=2, padding=1)
        self.context1 = ContextModule(16, 16)
        
        # Encoder Layer 2
        self.enc2 = nn.Conv3d(16, 32, kernel_size=(3,3,3), stride=2, padding=1)
        self.context2 = ContextModule(32, 32)
        
        # Encoder Layer 3
        self.enc3 = nn.Conv3d(32, 64, kernel_size=(3,3,3), stride=2, padding=1)
        self.context3 = ContextModule(64, 64)
        
        # Encoder Layer 4
        self.enc4 = nn.Conv3d(64, 128, kernel_size=(3,3,3), stride=2, padding=1)
        self.context4 = ContextModule(128, 128)
        
        # Bottleneck
        self.bottleneck = nn.Conv3d(128, 256, kernel_size=(3,3,3), stride=2, padding=1)
        ...
        # Decoding Layer 4
        self.final_conv = nn.Conv3d(32, num_classes, kernel_size=(3,3,3), stride=2, padding=1)
