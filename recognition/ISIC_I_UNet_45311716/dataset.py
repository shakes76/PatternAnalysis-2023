...
class UNetData():
    def __init__(self, path, height, width):
        ...
        valid_loader = torch.utils.data.DataLoader(valid_set, batch_size=16, shuffle=True)

        # Truth dataset
        valid_t_set = torchvision.datasets.ImageFolder(root=path+'validation_masks', transform=mask_transform)
        valid_t_loader = torch.utils.data.DataLoader(valid_t_set, batch_size=16, shuffle=True)
